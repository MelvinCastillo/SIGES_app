...
namespace CAID
{
  public class SMX00520 : Page
  {
    ...
    protected HtmlSelect cmbMedicoFiltro;
    ...
    private string ESPECIALISTA
    {
      get
      {
        return this.cmbMedicoFiltro.Value.ToString().ToUpper();
      }
    }
    ...
    private string ESTATUS
    {
      get
      {
        return this.cmbEstatus.Value.ToString().ToUpper();
      }
    }
    ...
    private void _Iniciar()
    {
      ...
      this.cmbEspecialidad.Items.Add(new ListItem("-- TODOS --", string.Empty));
      ...
      this.cmbMedicoFiltro.Items.Clear();
      this.cmbMedicoFiltro.Items.Add(new ListItem("-- TODOS --", string.Empty));
      foreach (DataRow row in (InternalDataCollectionBase) DB.ExecuteAdapter("SMX00300S3", this.xQueryParameters, this.xQueryValues, CommandType.StoredProcedure).Rows)
      {
        ...
        this.cmbMedicoFiltro.Items.Add(new ListItem(row["FULLNAME"].ToString(), row["ROWGUID"].ToString()));
      }
      this.xQueryParameters.Clear();
      this.xQueryValues.Clear();
      this.xQueryParameters.Add((object) "@NOMBRE");
    ...
    {
      this.cmbMedicoFiltro.Value = string.Empty;
