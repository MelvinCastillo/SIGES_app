...
namespace CAID
{
  public class HCL00010 : Page
  {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      ...
      foreach (DataRow row in (InternalDataCollectionBase) DB.ExecuteAdapter(string.Format("SMX30026S1 @NOMBRE = '{0}', @ROWSGXID = '{1}'", (object) "%%", this.Session["CURRENTUSR_SGXID"])).Rows)
    ...
    protected void _Save(object sender, EventArgs e)
    {
      ...
      if (!((Site_Master) this.Master).Crear_ActoMédico(xREFGUID, xTERGUID, xESPGUID))
        return;
      if (DB.ExecuteNonQuery(string.Format("[HCL00010SI] '{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}','{8}'", (object) str1, (object) str2, (object) str3, (object) text, (object) str4, (object) xREFGUID, (object) xTERGUID, (object) str5, (object) str6)))
        this.Validar();
    }
    ...
    private void Validar()
    {
      ...
      this.rpHistorico.DataSource = (object) DB.ExecuteAdapter(string.Format("HCL00010H @REFGUID = '{0}', @ROWSGXID = '{1}', @TIPO = '{2}'", this.Session["CurrentPAX_ROWGUID"], this.Session["CURRENTUSR_SGXID"], (object) "ALL"));
