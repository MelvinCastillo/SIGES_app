...
namespace CAID
{
  public class HCL00029 : Page
  {
    ...
    protected CheckBoxList ckMetodo;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      ...
      this.ckMetodo.Items.Clear();
      foreach (DataRow row in (InternalDataCollectionBase) DB.ExecuteAdapter("SELECT * FROM vMETODO_INTERVENCION_OCUPACIONAL ORDER BY ORDEN").Rows)
        this.ckMetodo.Items.Add(new ListItem(row["NOMBRE"].ToString(), row["ROWGUID"].ToString()));
    ...
    protected void _Save(object sender, EventArgs e)
    {
      if (!Convert.ToBoolean(this.Session["CurrentUSR_ROLEDITAR"].ToString()))
        return;
      string str1 = "";
      for (int index = 0; index < this.ckMetodo.Items.Count; ++index)
      {
        if (this.ckMetodo.Items[index].Selected)
          str1 = str1 + "," + this.ckMetodo.Items[index].Value.Trim();
      }
      if (str1 != "")
      ...
      if (!((Site_Master) this.Master).Crear_ActoMédico(xREFGUID, xTERGUID, xESPGUID))
        return;
      if (DB.ExecuteNonQuery(string.Format("[HCL00029SI] '{0}','{1}','{2}','{3}','{4}','{5}'", (object) str1, (object) str2, (object) xREFGUID, (object) xTERGUID, (object) str3, (object) str4)))
      {
    ...
    {
      DataTable dataTable1 = DB.ExecuteAdapter(string.Format("HCL00029V @TERGUID = '{0}', @ROWSGXID = '{1}', @REFGUID = '{2}'", this.Session["CurrentUSR_TERID"], this.Session["CURRENTUSR_SGXID"], this.Session["CurrentPAX_ROWGUID"]));
      ...
      string[] strArray = dataTable1.Rows[0]["METODO"].ToString().Split(chArray);
      if (strArray[0] != "")
      {
        foreach (string str in strArray)
          this.ckMetodo.Items.FindByValue(str).Selected = true;
      }
