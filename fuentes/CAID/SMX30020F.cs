...
namespace CAID
{
  public class SMX30020F : Page
  {
    ...
    protected HtmlSelect cmbFrecuente;
    protected RequiredFieldValidator valcmbFrecuente;
    ...
    private string FRECUENTE
    {
      get
      {
        return this.cmbFrecuente.Value.ToString().Trim().ToUpper();
      }
    }
    ...
    {
      this.cmbFrecuente.Items.Clear();
      this.cmbFrecuente.Items.Add(new ListItem("-- SELECCIONE --", string.Empty));
      foreach (DataRow row in (InternalDataCollectionBase) DB.ExecuteAdapter(string.Format("SMX30020S1 @NOMBRE = '{0}', @ROWSGXID = '{1}'", (object) "%%", this.Session["CURRENTUSR_SGXID"])).Rows)
        this.cmbFrecuente.Items.Add(new ListItem(row["NOMBRE"].ToString() + " (" + row["CODIGO1"].ToString() + ")", row["ROWGUID"].ToString()));
      ...
      else
      {
        ...
        if (dataRow == null)
          return;
        this.cmbFrecuente.Value = dataRow["FRECUENTE"].ToString();
    ...
    {
      this.cmbFrecuente.Value = string.Empty;
    }
    ...
    protected void _Save(object sender, EventArgs e)
    {
      this.xQueryParameters.Clear();
      this.xQueryValues.Clear();
      this.xQueryParameters.Add((object) "@FRECUENTE");
      this.xQueryValues.Add((object) this.FRECUENTE);
      this.xQueryParameters.Add((object) "@ROWGUID");
      this.xQueryValues.Add((object) this.FRECUENTE);
      this.xQueryParameters.Add((object) "@ROWUSERID");
      this.xQueryValues.Add((object) this.ROWUSERID);
