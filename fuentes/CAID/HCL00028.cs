...
namespace CAID
{
  public class HCL00028 : Page
  {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      ...
      foreach (DataRow row in (InternalDataCollectionBase) DB.ExecuteAdapter(string.Format("select * from vTF_ESCALAS_ASHWHORT order by valor")).Rows)
    ...
    protected void _Save(object sender, EventArgs e)
    {
      ...
      if (!((Site_Master) this.Master).Crear_ActoMédico(xREFGUID, xTERGUID, xESPGUID))
        return;
      if (DB.ExecuteNonQuery(string.Format("[HCL00028SI] '{0}','{1}','{2}','{3}','{4}','{5}','{6}'", (object) str1, (object) str2, (object) xREFGUID, (object) xTERGUID, (object) str3, (object) str4, (object) str5)))
        this.Validar();
    }
    ...
    private void Validar()
    {
      ...
      this.rpHistorico.DataSource = (object) DB.ExecuteAdapter(string.Format("HCL00028H @REFGUID = '{0}', @ROWSGXID = '{1}'", this.Session["CurrentPAX_ROWGUID"], this.Session["CURRENTUSR_SGXID"]));
