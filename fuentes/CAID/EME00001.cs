...
namespace CAID
{
  public class EME00001 : Page
  {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      ...
      foreach (DataRow row in (InternalDataCollectionBase) DB.ExecuteAdapter("SELECT * FROM vTIPOS_EMERGENCIA").Rows)
        this.cmbObjetivo.Items.Add(new ListItem(row["NOMBRE"].ToString(), row["ROWGUID"].ToString()));
      this.cmbObjetivo.Value = "004";
    ...
    protected void _New(object sender, EventArgs e)
    {
      ...
      this.cmbObjetivo.Disabled = false;
      this.txtEVALUACION.ReadOnly = false;
    }
    ...
    protected void _Save(object sender, EventArgs e)
    {
      if (!Convert.ToBoolean(this.Session["CurrentUSR_ROLEDITAR"].ToString()))
        return;
      if (!DB.ExecuteNonQuery(string.Format("[EME00001SI] '{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}'", (object) this.cmbObjetivo.Value.ToString(), (object) this.cmbObjetivo.Value.ToString(), (object) this.txtEVALUACION.Text.Trim(), (object) this.Session["CurrentPAX_ROWGUID"].ToString(), (object) this.Session["CurrentUSR_TERID"].ToString(), (object) this.cmbTipo.Value.ToString(), (object) this.Session["CurrentUSR_ROWGUID"].ToString(), (object) this.Session["CURRENTUSR_SGXID"].ToString())))
        return;
