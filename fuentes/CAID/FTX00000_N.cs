...
namespace CAID
{
  public class FTX00000_N : Page
  {
    ...
    protected HtmlSelect cmbTNotasContables;
    protected RequiredFieldValidator valcmbTNotasContables;
    protected HtmlInputText txtTIPONCDETALLES;
    protected HtmlInputText txtTIPONCPACIENTE;
    protected RequiredFieldValidator valtxtTIPONCPACIENTE;
    protected HtmlInputText txtTIPONCSEGURO;
    protected RequiredFieldValidator valtxtTIPONCSEGURO;
    ...
    private string TIPONCID
    {
      get
      {
        return this.cmbTNotasContables.Value.ToString().Trim().ToUpper();
      }
    }

    private string TIPONCDETALLES
    {
      get
      {
        return this.txtTIPONCDETALLES.Value.ToString().ToUpper();
      }
    }

    private double TIPONCSEGURO
    {
      get
      {
        return Convert.ToDouble(this.txtTIPONCSEGURO.Value);
      }
    }

    private double TIPONCPACIENTE
    {
      get
      {
        return Convert.ToDouble(this.txtTIPONCPACIENTE.Value);
      }
    }
    ...
    private void _Iniciar()
    {
      ...
      foreach (DataRow row in (InternalDataCollectionBase) DB.ExecuteAdapter("FTX00000S1", this.xQueryParameters, this.xQueryValues, CommandType.StoredProcedure).Rows)
        this.cmbFactura.Items.Add(new ListItem(row["NUMERO"].ToString() + " - " + row["NOMBRE"].ToString(), row["ROWGUID"].ToString()));
      this.cmbTNotasContables.Items.Clear();
      this.cmbTNotasContables.Items.Add(new ListItem("-- SELECCIONE --", string.Empty));
      foreach (DataRow row in (InternalDataCollectionBase) DB.ExecuteAdapter("SMX30024S1", this.xQueryParameters, this.xQueryValues, CommandType.StoredProcedure).Rows)
        this.cmbTNotasContables.Items.Add(new ListItem(row["NOMBRE"].ToString(), row["ROWGUID"].ToString()));
    ...
    private void _IniciarControles()
    {
      ...
      this.cmbTNotasContables.Value = string.Empty;
      this.txtTIPONCDETALLES.Value = string.Empty;
      this.txtTIPONCSEGURO.Value = "0.00";
      this.txtTIPONCPACIENTE.Value = "0.00";
    }
    ...
    {
      double num = this.TIPONCSEGURO + this.TIPONCPACIENTE;
      this.xQueryParameters.Clear();
      this.xQueryValues.Clear();
      this.xQueryParameters.Add((object) "@FACTURAID");
      this.xQueryValues.Add((object) this.FACTURAID);
      this.xQueryParameters.Add((object) "@TIPONCID");
      this.xQueryValues.Add((object) this.TIPONCID);
      this.xQueryParameters.Add((object) "@TIPONCDETALLES");
      this.xQueryValues.Add((object) this.TIPONCDETALLES);
      this.xQueryParameters.Add((object) "@TIPONCNETO");
      this.xQueryValues.Add((object) num);
      this.xQueryParameters.Add((object) "@TIPONCSEGURO");
      this.xQueryValues.Add((object) this.TIPONCSEGURO);
      this.xQueryParameters.Add((object) "@TIPONCPACIENTE");
      this.xQueryValues.Add((object) this.TIPONCPACIENTE);
      this.xQueryParameters.Add((object) "@ROWGUID");
      this.xQueryValues.Add((object) this.ROWGUID);
      this.xQueryParameters.Add((object) "@ROWUSERID");
      this.xQueryValues.Add((object) this.ROWUSERID);
