...
namespace CAID
{
  public class SMX00003 : Page
  {
    ...
    protected HtmlSelect cmbProvincia;
    ...
    private string PROVINCIAID
    {
      get
      {
        return this.cmbProvincia.Value.ToString().Trim().ToUpper();
      }
    }
    ...
    private void _Iniciar()
    {
      ...
      this.xQueryValues.Add((object) this.Session["CURRENTUSR_SGXID"].ToString());
      this.cmbProvincia.Items.Clear();
      this.cmbProvincia.Items.Add(new ListItem("-- SELECCIONE --", string.Empty));
      foreach (DataRow row in (InternalDataCollectionBase) DB.ExecuteAdapter("SMX00002S1", this.xQueryParameters, this.xQueryValues, CommandType.StoredProcedure).Rows)
        this.cmbProvincia.Items.Add(new ListItem(row["NOMBRE"].ToString(), row["ROWGUID"].ToString()));
      ...
      else
      {
        ...
        this.txtCodigo.Value = dataRow["CODIGOID"].ToString();
        this.cmbProvincia.Value = dataRow["PROVINCIAID"].ToString();
